...
mod test {
    ...
    fn test_widths() {
        ...
        expect_eq(lurk_main.width(), expect!["82"]);
        expect_eq(eval.width(), expect!["138"]);
        expect_eq(eval_comm_unop.width(), expect!["86"]);
        expect_eq(eval_hide.width(), expect!["102"]);
        expect_eq(eval_unop.width(), expect!["63"]);
        expect_eq(eval_binop_num.width(), expect!["89"]);
        expect_eq(eval_binop_misc.width(), expect!["58"]);
        expect_eq(eval_begin.width(), expect!["60"]);
        expect_eq(eval_let.width(), expect!["80"]);
        expect_eq(eval_letrec.width(), expect!["84"]);
        expect_eq(open_comm.width(), expect!["49"]);
        expect_eq(equal.width(), expect!["70"]);
        expect_eq(equal_inner.width(), expect!["57"]);
        expect_eq(car_cdr.width(), expect!["52"]);
        expect_eq(apply.width(), expect!["86"]);
