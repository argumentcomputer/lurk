...
/// When `lang_funcs` is empty, `eval_coroutine_expr` shouldn't be called.
/// ```ignore
    ...
    func!(
        fn ingress(tag_full: [8], digest: [8]): [2] {
            ...
            match tag {
                ...
                Tag::MutualThunk => {
                    let (ptr_tag_full: [8], ptr_digest: [8]) = preimg(hash2, digest);
                    let (ptr_tag, ptr) = call(ingress, ptr_tag_full, ptr_digest);
                    let ptr = store(ptr_tag, ptr);
                    return (tag, ptr)
                }
                Tag::Fun => {
    ...
    func!(
        fn egress(tag, val): [9] {
            match tag {
                ...
                Tag::MutualThunk => {
                    let (ptr_tag, ptr) = load(val);
                    let (ptr_tag, ptr_digest: [8]) = call(egress, ptr_tag, ptr);
                    let padding = [0; 7];
                    let digest: [8] = call(hash2, ptr_tag, padding, ptr_digest);
                    return (tag, digest)
                }
                Tag::Fun => {
...
pub fn hash2<F>() -> FuncE<F> {
    func!(
        invertible fn hash2(preimg: [16]): [8] {
            let img: [8] = extern_call(hasher2, preimg);
            return img
        }
    )
}
    ...
    func!(
        fn collect_mutual_env(binds_tag, binds): [2] {
            ...
            match binds_tag {
                InternalTag::Nil => {
                    let one = 1;
                    return (one, zero) // (success, empty env)
                }
                Tag::Cons => {
                    let cons_tag = Tag::Cons;
                    ...
                    match var_tag {
                        Tag::Sym, Tag::Builtin, Tag::Coroutine => {
                            ...
                            return (one, env) // (success, extended env)
                        }
                    };
...
pub fn extend_env_with_mutuals<F: AbstractField>() -> FuncE<F> {
    func!(
        fn extend_env_with_mutuals(mutual_env, env): [1] {
            if !mutual_env {
                return env
            }
            let (var_tag, var, val_tag, val, mutual_env) = load(mutual_env);
            let env = call(extend_env_with_mutuals, mutual_env, env);
            let mutual_thunk_tag = Tag::MutualThunk;
            let mutual_thunk = store(val_tag, val);
            let env = store(var_tag, var, mutual_thunk_tag, mutual_thunk, env);
            return env
        }
    )
}
...
pub fn eval_mutual_bindings<F: AbstractField>() -> FuncE<F> {
    func!(
        partial fn eval_mutual_bindings(mutual_env, env): [2] {
            if !mutual_env {
                let env_tag = Tag::Env;
            ...
            let (res_tag, res) = call(eval_mutual_bindings, mutual_env, env);
            return (res_tag, res)
        }
    )
}
...
pub fn eval_mutrec<F: AbstractField>() -> FuncE<F> {
    func!(
        partial fn eval_mutrec(binds_tag, binds, body_tag, body, env): [2] {
            ...
            let env = call(extend_env_with_mutuals, mutual_env_or_err, env);
            // preemptively evaluate each binding value for side-effects, error detection and memoization
            let (res_tag, res) = call(eval_mutual_bindings, mutual_env_or_err, env);
            match res_tag {
                Tag::Err => {
                    return (res_tag, res)
                }
...
mod test {
    ...
    fn test_widths() {
        ...
        let hash2 = FuncChip::from_name("hash2", toplevel);
        ...
        expect_eq(extend_env_with_mutuals.width(), expect!["29"]);
        ...
        expect_eq(hash2.width(), expect!["333"]);
