...
mod test {
    ...
    fn test_widths() {
        ...
        expect_eq(lurk_main.width(), expect!["88"]);
        expect_eq(eval.width(), expect!["144"]);
        expect_eq(eval_comm_unop.width(), expect!["92"]);
        expect_eq(eval_hide.width(), expect!["108"]);
        expect_eq(eval_unop.width(), expect!["69"]);
        expect_eq(eval_binop_num.width(), expect!["95"]);
        expect_eq(eval_binop_misc.width(), expect!["64"]);
        expect_eq(eval_begin.width(), expect!["66"]);
        expect_eq(eval_let.width(), expect!["86"]);
        expect_eq(eval_letrec.width(), expect!["90"]);
        expect_eq(open_comm.width(), expect!["49"]);
        expect_eq(equal.width(), expect!["76"]);
        expect_eq(equal_inner.width(), expect!["57"]);
        expect_eq(car_cdr.width(), expect!["58"]);
        expect_eq(apply.width(), expect!["92"]);
