...
use crate::stark::config::{Domain, PackedChallenge, PackedVal, StarkGenericConfig, Val};
    ...
    // A: Air<SymbolicAirBuilder<Val<SC>>> + for<'a> Air<ProverConstraintFolder<'a, SC>>,
{
    ...
    let multiplicities = {
        ...
        let values: Vec<_> = multiplicities
            .into_par_iter()
            .flat_map(|m| m.rlc(mu))
            .collect();
        ...
        let multiplicities =
        //
        // // for each provide interaction,
        // for (i, (m, provide)) in multiplicities.into_iter().zip(provides).enumerate() {
        //
        // }
