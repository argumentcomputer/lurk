...
mod tests {
    ...
    fn set_pos(l: &LambdaSyntax, p: Pos) -> LambdaSyntax {
        match l.clone() {
            LambdaSyntax::Ident(_, a) => LambdaSyntax::Ident(p, a),
            LambdaSyntax::Int(_, a, b) => LambdaSyntax::Int(p, a, b),
            LambdaSyntax::Float(_, a) => LambdaSyntax::Float(p, a),
            LambdaSyntax::Char(_, a) => LambdaSyntax::Char(p, a),
            LambdaSyntax::String(_, a) => LambdaSyntax::String(p, a),
            LambdaSyntax::Record(_, a, b) => {
                LambdaSyntax::Record(p, a, b.into_iter().map(|s| set_pos(&s, p)).collect())
            }
            LambdaSyntax::Setglobal(_, a, b) => {
                LambdaSyntax::Setglobal(p, set_pos(&a, p).into(), set_pos(&b, p).into())
            }
            LambdaSyntax::Seq(_, xs) => {
                LambdaSyntax::Seq(p, xs.into_iter().map(|s| set_pos(&s, p)).collect())
            }
            LambdaSyntax::Makeblock(_, a, b) => {
                LambdaSyntax::Makeblock(p, a, b.into_iter().map(|s| set_pos(&s, p)).collect())
            }
            LambdaSyntax::Let(_, binds, body) | LambdaSyntax::Letrec(_, binds, body) => {
                let binds = binds
                    .into_iter()
                    .map(|(a, b)| (set_pos(&a, p), set_pos(&b, p)))
                    .collect();
                let body = set_pos(&body, p).into();
                match *l {
                    LambdaSyntax::Let(_, _, _) => LambdaSyntax::Let(p, binds, body),
                    LambdaSyntax::Letrec(_, _, _) => LambdaSyntax::Letrec(p, binds, body),
                    _ => unreachable!(),
                }
            }
            LambdaSyntax::Function(_, args, body) => LambdaSyntax::Function(
                p,
                args.into_iter().map(|a| set_pos(&a, p)).collect(),
                set_pos(&body, p).into(),
            ),
            LambdaSyntax::Apply(_, func, args) => LambdaSyntax::Apply(
                p,
                set_pos(&func, p).into(),
                args.into_iter().map(|a| set_pos(&a, p)).collect(),
            ),
            LambdaSyntax::FallbackPrimitive(_, a, b) => LambdaSyntax::FallbackPrimitive(
                p,
                a,
                b.into_iter().map(|s| set_pos(&s, p)).collect(),
            ),
            LambdaSyntax::FallbackLiteral(_, a) => LambdaSyntax::FallbackLiteral(p, a),
        }
    }
    ...
    fn test_parser(input: &str, expected: Option<LambdaSyntax>) {
        ...
        if let Some(expected) = expected {
            let pos = expected.get_pos();
            assert_eq!(set_pos(&res, *pos), expected);
        }
    }
